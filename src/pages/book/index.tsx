...
function BookPage() {
  ...
  if (!book) {
    return (
      <Layout.Content className="flex justify-center items-center" aria-busy="true" aria-label="book not found message">
        <Result
          status="404"
          title="404"
      ...
      <article className="flex flex-col justify-between min-h-[300px] p-7 shadow-insetDark">
        ...
        <div className="mt-6 md:mt-10 w-full md:w-[300px] gap-2 m-auto">
          ...
          <div className="mt-5">
            ...
            {rent.status === "RESERVABLE" && <Reserve.Actions.ReserveBook bookId={book.id} aria-label="reserve book" />}
            {rent.status === "OUT_STOCK" && <Reserve.Actions.ReserveBook bookId={book.id} aria-label="reserve book" />}
          </div>
        </div>
      </article>
    </Col>
